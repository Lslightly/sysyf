...
const_init_val: exp #single | '{' '}' #arrayEmpty | '{' exp (',' exp)* '}' #arrayMul;
var_decl: btype var_def (',' var_def)* ';';
var_def: ident (LBRACKET INT? RBRACKET)? ('=' const_init_val)?;
func_def: (btype | VOID_T) ident '(' (func_fparam (',' func_fparam)*)? ')' block;
...
exp: otherexp #otherexpAlt
    | (PLUS | MINUS | NOT) exp #unary
    | exp (MUL | DIV | MOD) exp #binaryHigh
    | exp (PLUS | MINUS) exp #binaryLow;

otherexp: lval | number | func_call | '(' exp ')';
